...
def read_gsm(n):
	...
	for l in f:
		...
		if pval1<0.05 and pval2<0.05:
			...
			#by_query[q].append((g, pval1, pval2, np.log(avgexpr_h/a_mean))) #mean, test
...
def is_not_gene(g, choice, qid):
	if choice=="EA":
		...
		if qid==5 and g=="PTPRC":
			return False
		if qid==4 and (g=="PTPRC" or g=="CORO1A" or g=="CD52"):
			return False
		if qid==1 and g=="CD52":
			return False
		return True
...
if __name__=="__main__":
	choice = "EA"
	_, _, _, gene_sum = pick_genes_by_total.read("/media/qian/T7/GSE210616/DE.EA/with.GZMB/cell.type.expr.q%s.%s.more.joined.txt" % (sys.argv[1], choice), choice)
	...
	if choice=="EA":
		...
		#gene_sum.sort(key=itemgetter(5), reverse=True)
		#genes_EA = set([g[0] for g in gene_sum[:20]])
		gene_sum.sort(key=itemgetter(1), reverse=True)
